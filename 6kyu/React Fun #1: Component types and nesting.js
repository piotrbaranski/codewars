/**What is React?
 ...
 const Title=()=><h1>Hello</h1>;
 const Nest=()=><div> <Title ></Title> </div>; //nesting Title
 Simple, isn't it? :)

 Here, Nest became a parent component of Title, which in turn, is referred to as the child of Nest.

 Notice the <div> tag. In this case, it's not necessary, we could just return the <Title ></Title> directly but if we want to have more components returned by one, or repeat the same component, they all need to be wrapped within one tag.

 BAD: const Nest=()=><Title ></Title><Title ></Title>; This will cause an error.

 GOOD: const Nest=()=><div><Title ></Title><Title ></Title></div>;

 TIP: Always start component names with a capital letter. For example, <div ></div> is a DOM tag but <Title ></Title> would be recognised as a component and would require Title to be in scope.
 ...
 <Title ></Title> and <TitleClass ></TitleClass> are, in this case, doing the same thing - returning Hello wrapped with <h1>.
